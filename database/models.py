...
class Patungan(Base):
    ...
    discord_channel_id = Column(String(50), nullable=True) # Pastikan baris ini ada
    discord_role_id = Column(String(50), nullable=True)    # Pastikan baris ini ada
    use_script = Column(String, nullable=True)
    start_mode = Column(String, nullable=True)
    duration_hours = Column(Integer, default=24)
    start_schedule = Column(String, nullable=True)

class PatunganMaster(Base):
    __tablename__ = 'patungan_master'
    
    id = Column(Integer, primary_key=True)
    version = Column(String(10), unique=True, nullable=False)  # V1, V2, etc
    display_name = Column(String(100), nullable=False)
    duration_hours = Column(Integer, nullable=False)
    price_per_slot = Column(Integer, nullable=False)  # HARGA dari admin
    max_slots = Column(Integer, default=19)
...
class UserSlot(Base):
    ...
    patungan_version = Column(String(10), ForeignKey('patungan_master.version'), nullable=False)
    ...
    patungan = relationship("PatunganMaster", back_populates="slots")
...
class PriceHistory(Base):
    ...
    # Relationships
    patungan = relationship("PatunganMaster", back_populates="price_history")
